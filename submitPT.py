...
dataset = "chembl"
job_type = "generate"     # "preprocess", "train", "generate", "test", "multiple_valid" or "valid_loss"
...
jobname = "test"      # used to create a sub directory
...
# set paths here
python_path = f"{home}/anaconda3/envs/GRELinker-env/bin/python"
grelinker_path = f"pre-training/"
...
params = {
    ...
    "batch_size": 1000,
    "block_size": 1000,
    "generation_epoch": [5], #['None'] + [a for a in range(5, 201, 5)],  # <-- which model to use (i.e. which epoch) must be a number for "generate" and a list for "multiple_valid" and "valid_loss"
    ...
    for job_idx in range(jobdir_start_idx, jobdir_end_idx):
        ...
        try:
            os.makedirs(params["job_dir"],
                        exist_ok=bool(job_type in ["generate", "test", "multiple_valid", "valid_loss"] or force_overwrite))
            print(
...
def write_submission_script(job_dir, job_idx, job_type, max_n_nodes, runtime,
    ...
    with open(submit_filename, "w") as submit_file:
        ...
        submit_file.write(f"{python_bin_path} {grelinker_path}main.py --job-dir {job_dir}")
...
def check_paths():
    ...
    """
    for path in [python_path, grelinker_path, data_path]:
        if "path/to/" in path:
            print("!!!")
